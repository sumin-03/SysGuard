...
| TASK-B-022 | P1 | **DONE** | Agent self-config rewrites are informational. Once rename collection landed (`TASK-A-015`), a read-only session graded UNSAFE because Claude replaces `~/.claude.json` (and migrates `installed_plugins.json`) several times per session. The atomic-replace / sibling-migration shape by the owning agent is now informational with its own report block; direct writes, foreign sources and `RENAME_EXCHANGE` still escalate. Accepted false negative documented. Also fixes `has_critical` trusting a recorded severity this policy has downgraded. |
| TASK-B-021 | P2 | **DONE** | Report design pass focused on legibility: path typography (muted directory, firm basename) as the signature, an 11-size type scale reduced to 4 steps, a system font stack that no longer falls back from `Segoe UI`, the verdict colour as a document-wide rail, and accessible white-on-colour variants (the brand orange gave 2.57:1). |
| TASK-B-020 | P2 | **DONE** | Presentation demo scenario (`docs/demo_scenario.md`) with a four-act runnable script, Q&A prep, and a fallback plan; plus `demo/agent_persistence_simulator.sh`, which exercises the persistence rules against a sandbox HOME so the real `~/.bashrc` is never touched. |
| TASK-B-019 | P2 | **DONE** | HTML report readability: verdict summary chips + a "Why this verdict" line above the sections, `<details>` collapsing for bulky low-signal blocks (with a print stylesheet that re-expands them), compact metadata grid, alert table restructured (reason moved under the target, non-critical outbound split out as evidence). |
| TASK-B-018 | P1 | **DONE** | GUI responsiveness + presentation: the session list re-analysed every log inline on the Tk thread (measured **15.6 s** freeze, including at startup), now cached by (mtime, size) and computed off-thread; live event counter during collection; `ttk.Treeview` list with sortable columns and verdict tints; selected-session badge. |
| TASK-A-015 | P1 | **DONE** | Attach `sys_enter_rename` and `sys_enter_renameat`. Only `renameat2` is traced today, but glibc `rename()` issues the `rename` syscall (82) — measured: a 5,143-event real session recorded **zero** rename events. The whole rename-destination policy (protected / persistence / outside, `RENAME_EXCHANGE`) is therefore dead code in production, and the atomic-replace bypass it was built to stop would go undetected. Blocks project-local agent-config detection. |
...
| TASK-B-017 | P2 | **DONE** | Severity calibration: `outside-project-write` high→**medium** (it fires only after protected/persistence/bookkeeping are ruled out, and was outranking `file-unlink`), and `unsafe-chmod` unified at **high** across `execve`/`fchmodat`. Verdicts unchanged — only `critical` escalates. GUI reverted to not passing `--tool-tmp` (CLI-only opt-in). |
| TASK-B-016 | P2 | **DONE** | Recognize Claude background-job bookkeeping (`~/.claude/jobs/` `pins.json` / `<job>/state.json[.tmp.<hex>]` / `<job>/timeline.jsonl`) as runtime noise — matched as specific files, not as a subtree. |
