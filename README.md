...
**알려진 한계 — agent 설정 원자적 교체(`TASK-B-022`).** Claude Code는 자기 설정(`~/.claude.json`, `~/.claude/settings*.json`, `~/.claude/plugins/installed_plugins.json`)을 `<파일>.tmp.<pid>.<hex>` 에 쓴 뒤 rename 하는 방식으로만 갱신하며, 아무 작업도 하지 않은 세션에서도 여러 번 발생한다(실측: staging+rename 4회, 직접 쓰기 0회). rename 수집(`TASK-A-015`)이 들어오자 **모든 정상 세션이 `UNSAFE`** 가 되어, 이 형태를 **정보성으로 분류**하기로 결정했다.
...
플러그인 레지스트리(`installed_plugins.json`)는 staging 대신 **스키마 마이그레이션**으로 갱신된다 — 관측된 모든 세션이 `installed_plugins_v2.json` 을 같은 디렉터리에서 rename 한다. 그래서 **같은 디렉터리의 버전 접미사 형제 파일**에서 오는 rename만 정보성으로 인정하며, 임의 파일·다른 디렉터리·다른 agent 는 모두 `UNSAFE` 로 남는다. 이 파일에 **직접 쓰기**도 `UNSAFE` 다(어떤 플러그인이 로드되는지 결정하므로 `settings.json` 과 같은 활성화 파일이다).
